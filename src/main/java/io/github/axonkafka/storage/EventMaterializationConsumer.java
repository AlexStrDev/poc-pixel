...
/**
 * Consumer dedicado para materialización asíncrona de eventos.
 */
@Slf4j
public class EventMaterializationConsumer {
    ...
    @KafkaListener(
        ...
        groupId = "${axon.kafka.materializer.group-id}",
    ...
    public void materializeEvent(
        ...
        try {
            log.debug("Materializando evento - Offset: {}", record.offset());
            ...
            if (count > 0) {
                log.debug("Evento ya materializado (idempotente)");
            ...
            log.debug("Evento materializado: aggregate={}, seq={}", 
                event.getAggregateIdentifier(), event.getSequenceNumber());
            
            acknowledgment.acknowledge();
            
        } catch (Exception e) {
            log.error("Error materializando evento - NO se hace acknowledge", e);
